...
module.exports.paginate = function(count, nPerPage, pageNumber, opts) {
  ...
  settings['prev_text'] = '<span class="glyphicon glyphicon-backward"></span>';
  settings['next_text'] = '<span class="glyphicon glyphicon-forward"></span>';
  settings['end_size'] = 1;
  settings['mid_size'] = 1;
  ...
  if (settings['prev_text'] && settings['current'] && 1 < settings['current']) {
    ...
    page_links.push('<li class="page-prev"><a href="' + link + settings["add_args"] + '">' + settings["prev_text"] + '</a></li>');
  }
  ...
  if (settings["prev_next"] && settings["current"] && (settings["current"] < settings["total"] || -1 === settings["total"])) {
    ...
    page_links.push('<li class="page-next"><a href="' + link + settings["add_args"] + '">' + settings["next_text"] + '</a></li>');
  }
  return '<ul class="pagination">' + page_links.join("\n") + '</ul>';
}
